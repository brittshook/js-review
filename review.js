...
const myStr = 'My dog is' + iAmAVariable + 'years old';

// 5.) Use any of the variables above or create new ones and print using string interpolation
const firstName = 'Britt';
const newStr = `My name is ${firstName}`;
...
const teenageWitch = 'Sabrina';
console.log(teenageWitch.charAt(4));

// Define Unicode in a comment, and get the Unicode using charCodeAt() of the third character in Teddy
// Unicode is a universal standard to encode characters from nearly all writing systems. This means that 
// characters, such as those from the latin alphabet or Chinese characters, are given a unicode that is then
// converted into binary code to be processed by computers. 
const bear = 'Teddy';
const unicode = bear.charCodeAt(2);
console.log(unicode);
...
// I already cached my first name in an early exercise, so I will reference that variable.
const lastName = 'Shook'
console.log(firstName + ' ' + lastName);

// Create a string and make it return true using startsWith()
const teachers = 'Tishana and Manara are the best!';
console.log(teachers.startsWith('Tishana'));

// Now use any variable with endsWith() and return false
console.log(bear.endsWith('bear'));

// Finish the following sentence. Use includes() and return true.
const ozgur = 'Once upon a time Ozgur looked up at the Moon, and there he saw the face of his long lost love.';
console.log(ozgur.includes('lost'));
...
console.log(pizzaSentence.slice(Math.floor(pizzaSentence.length / 2)));
...
console.log(splitTheBill.replace(/,|and /g, '').split(' '));
...
const angryTuan = "WTF!? I CAN'T DO THIS ANYMORE";
...
const sheThick = '            hello           ';
console.log(sheThick.trim());
...
for (let i = 0; i < 10; i++) {
  console.log(`The room spins ${i + 1} time${ i ? 's' : ''}`);
}

// Harry said that was so mean of Teo to do Ken dirty like that. Make Teo's vision spin 20 times FOR LOOP.
for (let i = 0; i < 20; i++) {
  console.log(`Teo's vision is spinning${', spinning'.repeat(i)}`);
}
...
perscholas.forEach(learner => {
  const ps = 'PS';
  console.log(ps + ' ' + learner);
})
...
// concat()
// Combines two or more arrays (can also use on strings)

// splice()
// Add or remove items from an array, mutating the original array

// slice()
// Return selected items from an array, and does not mutate the original array

// sort()
// Sorts array items in ascending order based on first character's unicode by default (or via specified comparison function)

// includes()
// Checks if array contains specific value (or string contains specific substring)

// indexOf()
// Returns first instance of specified value or -1 if not found
...
fruits.splice(fruits.length, 0, 'strawberry', 'kiwi', 'grape');
console.log(fruits);
...
const exoticFruits = ['starfruit', 'durian', 'dragonfruit'];
const fruitsGalore = fruits.concat(exoticFruits);
console.log(fruitsGalore);

// Print the last two exotic fruits without altering the newly concatenated array.
console.log(fruitsGalore.slice(-2));
...
const RafaelGetsCleanedUp = RafaelNoBadWords
  .split(' ')
  .filter(word => word !== 'badword')
  .join(' ');
